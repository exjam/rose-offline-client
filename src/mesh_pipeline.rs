...
pub const MESH_VIEW_BIND_GROUP_HANDLE: HandleUntyped =
    HandleUntyped::weak_from_u64(Shader::TYPE_UUID, 1076678235888822571);

pub const MESH_STRUCT_HANDLE: HandleUntyped =
    HandleUntyped::weak_from_u64(Shader::TYPE_UUID, 1506024101911992371);
...
impl Plugin for MeshRenderPlugin {
    fn build(&self, app: &mut App) {
        load_internal_asset!(
            app,
            MESH_STRUCT_HANDLE,
            "shaders/mesh_struct.wgsl",
            Shader::from_wgsl
        );
        load_internal_asset!(
            app,
            MESH_VIEW_BIND_GROUP_HANDLE,
            "shaders/mesh_view_bind_group.wgsl",
            Shader::from_wgsl
        );
        load_internal_asset!(
            app,
...
pub fn extract_meshes(
    ...
    caster_query: Query<(Entity, &ComputedVisibility, &GlobalTransform, &Handle<Mesh>)>,
) {
    let mut caster_values = Vec::with_capacity(*previous_caster_len);
    for (entity, computed_visibility, transform, handle) in caster_query.iter() {
...
impl<const I: usize> EntityRenderCommand for SetMeshViewBindGroup<I> {
    type Param = SQuery<(Read<ViewUniformOffset>, Read<MeshViewBindGroup>)>;
    #[inline]
    fn render<'w>(
        view: Entity,
        _item: Entity,
