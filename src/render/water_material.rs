...
use bevy::{
    ...
    pbr::{
        extract_materials, prepare_materials, queue_material_meshes, DrawMesh, ExtractedMaterials,
        MaterialPipeline, RenderMaterials, SetMaterialBindGroup, SetMeshBindGroup,
        SetMeshViewBindGroup,
    },
    prelude::{
        AddAsset, AlphaMode, App, Assets, Commands, FromWorld, HandleUntyped, Image,
        IntoSystemAppConfig, IntoSystemConfig, Material, Mesh, Plugin, Res, Resource, Time, World,
    },
    reflect::TypeUuid,
    render::{
...
impl Plugin for WaterMaterialPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_asset::<WaterMaterial>()
            .add_plugin(ExtractComponentPlugin::<Handle<WaterMaterial>>::extract_visible());

        if let Ok(render_app) = app.get_sub_app_mut(RenderApp) {
            render_app
                .add_render_command::<Transparent3d, DrawWaterMaterial>()
                .add_render_command::<Opaque3d, DrawWaterMaterial>()
                .add_render_command::<AlphaMask3d, DrawWaterMaterial>()
                .init_resource::<MaterialPipeline<WaterMaterial>>()
                .init_resource::<ExtractedMaterials<WaterMaterial>>()
                .init_resource::<RenderMaterials<WaterMaterial>>()
                .init_resource::<SpecializedMeshPipelines<MaterialPipeline<WaterMaterial>>>()
                .add_system(extract_materials::<WaterMaterial>.in_schedule(ExtractSchedule))
                .add_system(
                    prepare_materials::<WaterMaterial>
                        .in_set(RenderSet::Prepare)
                        .after(PrepareAssetSet::PreAssetPrepare),
                )
                .add_system(
                    queue_material_meshes::<WaterMaterial, DrawWaterMaterial>
