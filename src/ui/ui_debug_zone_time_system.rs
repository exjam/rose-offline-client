    ...
    egui::Window::new("Zone Time")
        ...
        .show(egui_context.ctx_mut(), |ui| {
            ...
            egui::Grid::new("zone_time_cycle_grid")
                ...
                .show(ui, |ui| {
                    ui.label("Day Cycle:");
                    ui.label(format!("{}", zone_time.day_cycle));
                    ui.end_row();

                    ui.label("Morning Time:");
                    ui.label(format!("{}", zone_time.morning_time));
                    ui.end_row();

                    ui.label("Day Time:");
                    ui.label(format!("{}", zone_time.day_time));
                    ui.end_row();

                    ui.label("Evening Time:");
                    ui.label(format!("{}", zone_time.evening_time));
                    ui.end_row();

                    ui.label("Night Time:");
                    ui.label(format!("{}", zone_time.night_time));
                    ui.end_row();
                });
        });
}
